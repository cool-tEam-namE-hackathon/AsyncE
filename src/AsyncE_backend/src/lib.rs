...
use globals::{GROUPS, GROUP_INVITES, PRINCIPAL, USERS, VIDEOS};
...
fn pre_upgrade() {
    ...
    let principal_store = PRINCIPAL.get();
...
#[ic_cdk::post_upgrade]
fn post_upgrade() {
    let (user_store, group_store, videos_store, group_invites_store, principal_store) =
